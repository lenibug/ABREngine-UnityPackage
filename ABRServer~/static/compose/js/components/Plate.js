...
import { COMPOSITION_LOADER_ID } from './Components.js';
import { InputPuzzlePiece, AssignedInputPuzzlePiece } from "./PuzzlePiece.js";
import { uuid } from "../../../common/UUID.js";

// This defines the "topology" of the plate inputs - i.e., which
...
export const PlateInputTopology = {
    "Glyphs": [
        // Tier 1
        [
...
export function Plate(plateType, uuid, name, impressionData) {
    ...
    let $element = $('<div>', { class: 'plate rounded' })
        .data({
            uuid,
            plateType,
        });
    let $tower = $('<div>', {
        class: 'plate-tower rounded',
    });
    ...
    $element.append($('<div>', {
        class: 'plate-header rounded-top',
    ...
    let parameterTiers = PlateInputTopology[plateType];

    $element.append(PlateSummary(uuid, name, impressionData, inputValues, parameterTiers));
    ...
    $dropZone.droppable({
        ...
        drop: (evt, ui) => {
            ...
            if (validTypes.indexOf(droppedType) >= 0) {
                ...
                // If the plate hasn't been renamed by user (with the pencil icon),
...
function PlateSummary(uuid, name, impressionData, inputValues, parameterTiers) {
    let collapsed = false;
    ...
    let $el = $('<div>', {
        ...
        $('<div>', { class: 'impression-controls' }).append(
            $('<button>', {
                ...
                title: oldVisibility ? 'This plate is visible' : 'This plate is not shown',
            }).on('click', (evt) => {
            ...
            $('<button>', {
                class: 'rounded',
                title: 'Edit this plates\'s properties'
            }).append(
...
function duplicatePlate(oldUuid) {
...
function editPlate(uuid) {
    let $diEditor = $('<div>', {
        class: 'plate-editor',
    });

    if ($('.plate-editor').length > 0 && $('.plate-editor').dialog('isOpen')) {
    ...
    let visTag;
    let visBool = true;
    let vistype = globals.stateManager.state.impressions[uuid].plateType;
    if (globals.stateManager.state.impressions[uuid] && globals.stateManager.state.impressions[uuid].inputValues) {
     
        if(globals.stateManager.state.impressions[uuid].tags){
            tags.push(globals.stateManager.state.impressions[uuid].tags);
        }
        let values = globals.stateManager.state.impressions[uuid].inputValues;

        if(values.Colormap){
            colorTag = globals.stateManager._caches.visassets[values.Colormap.inputValue].tags;
        }
        if(values.Glyph){
            visTag = globals.stateManager._caches.visassets[values.Glyph.inputValue].tags;
        }else if(values.Texture){
            visTag = globals.stateManager._caches.visassets[values.Texture.inputValue].tags;
        }else if(values.Line){
            visTag = globals.stateManager._caches.visassets[values.Line.inputValue].tags;
        }else{ //no visasset attached to plate
            visTag = [''];
            visBool = false;
        }
    ...
    $diEditor.append($platetagEditor);
    if(values.Colormap){ // colormap is attached to plate
        let $colortagEditor = $('<label>', {
            id: 'ct-tags',
            text: 'Colormap tags:'
        }).append($('<input>', {
            type: 'text',
        ...
        $diEditor.append($colortagEditor);
    }
    if(visBool){ //visasset is attached to plate
        let $visassettagEditor = $('<label>', {
            id: 'va-tags',
            text: vistype+=' tags: ',
        }).append($('<input>', {
            type: 'text',
        ...
        $diEditor.append($visassettagEditor);
        }
    }
    ...
    let saveProperties = () => {
        ...
        if(values){
            if(values.Colormap !== undefined){
                let colorTags = $('#ct-tags input').val();

                if (colorTags.length > 0) {
                    colorTags = colorTags.split(',');
                }else if(colorTags == ''){
                    colorTags = [''];
                }
                let coloruuid = values.Colormap.inputValue;
                if(globals.stateManager._caches.visassets[coloruuid]){
                    globals.stateManager.updateAsset(coloruuid, colorTags);
                }
    
            }
        
            if(visBool){
                let visTags = $('#va-tags input').val();
                let visuuid;
            
                if (visTags.length > 0) {
                    visTags = visTags.split(',');
                }else if(visTags == ''){
                    visTags = [''];
                }
                if(values.Glyph){
                    visuuid = values.Glyph.inputValue;
                }else if(values.Texture){
                    visuuid = values.Texture.inputValue;
                }else if(values.Line){
                    visuuid = values.Line.inputValue;
                }
                if(globals.stateManager._caches.visassets[visuuid]){
                    globals.stateManager.updateAsset(visuuid, visTags);
